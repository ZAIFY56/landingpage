const Card = ({ icon, title, description, className = '', hoverEffect = true, children }) => {
  return (
    ...
    >
      ...
      {title && <h3 className="text-md mt-2 2xl:text-[20px] font-bold text-center mb-2">{title}</h3>}
      {description && <p className="text-sm text-gray-700 2xl:text-[18px]  2xl:mx-[3rem] text-center">{description}</p>}
      {children}
    </div>
  );
};
